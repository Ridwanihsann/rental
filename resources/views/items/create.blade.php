...
@section('content')
    <x-page-header title="Tambah Barang" :backUrl="route('items.index')" />
    ...
    <form action="{{ route('items.store') }}" method="POST" enctype="multipart/form-data" id="create-form">
        ...
        <div class="form-group">
            ...
            <div style="position: relative;">
                ...
                <input type="number" name="daily_price" id="daily_price" class="form-control" placeholder="0"
                    value="{{ old('daily_price') }}" style="padding-left: 2.5rem;" min="0" required>
            </div>
            @error('daily_price')
                <span style="color: var(--color-danger); font-size: 0.75rem;">{{ $message }}</span>
            @enderror
        ...
        <div class="form-group">
            ...
            <textarea name="description" id="description" class="form-control" rows="3"
                placeholder="Deskripsi barang, kondisi, kelengkapan, dll...">{{ old('description') }}</textarea>
            @error('description')
                <span style="color: var(--color-danger); font-size: 0.75rem;">{{ $message }}</span>
            @enderror
        </div>
